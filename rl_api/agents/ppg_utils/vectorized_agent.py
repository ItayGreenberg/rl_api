from rl_api.networks.networks import (
    ActorNetwork, CriticNetwork, PPGPolicyNetwork, PPGValueNetwork
)

from rl_api.agents.ppg_utils.buffer import RolloutBufferVec

from rl_api.agents.ppg_utils.configs import (
    DimensionConfig, PPOConfig, PPGConfig, EntropySchedulerConfig, BufferConfig,
    TrainingConfig, EvalConfig, LoggingConfig, SavingConfig,
)


from rl_api.agents.ppg_utils.logger import Logger
...
class VectorizedPPGAgent:

    def __init__(
        self,
        policy_net: PPGPolicyNetwork,          # encoder + action head + aux-value head
        value_net:  PPGValueNetwork,           # separate encoder + value headizer,
        ...
        self.opt_aux     = aux_optimizer
        self.opt_critic  = critic_optimizer
    ...
    def _unpack_ppg_cfg(self):
        pp = self.ppg_cfg
        self.N_pi         = pp.n_pi
        self.policy_epochs= pp.policy_epochs
        self.aux_epochs   = pp.aux_epochs
        self.beta_kl      = pp.beta_kl
    ...
    def _setup_buffers(self):
        ...
        self.ppo_buf = RolloutBufferVec(
            ...
            store_old_logits=True,
            normalize_returns=True
        )

        # AUX buffer on CPU pinned (stream to GPU)
        self.aux_buf = RolloutBufferVec(
            buffer_size = buf.buffer_size * self.ppg_cfg.n_pi,
            obs_shape   = self.obs_shape,
            context_dim = self.context_dim,
            action_dim  = self.action_dim,
            gamma       = self.gamma,
    ...
    def _setup_tracking(self):
        ...
        self._iter_in_phase = 0 # ppo updates done
    ...
    def _reset_sample_epoch_stats(self):  
        self.list_stats = dict(
            ...
            clip_frac            = [], approx_kl     = [], true_kl         = [],
            explained_var        = [],

            policy_enc_grad      = [], actor_grad    = [], 
            value_enc_grad       = [], critic_grad   = [],
    ...
    def _gather_sample_stats(self) -> dict:
        ...
        num_batches_per_epoch = (self.buf_cfg.buffer_size + self.buf_cfg.ppo_batch_size - 1) // self.buf_cfg.ppo_batch_size
        num_batches = num_batches_per_epoch * self.ppg_cfg.policy_epochs * self.logging_cfg.log_interval
    ...
    def calculate_value_loss(self, old_values: Tensor, values: Tensor, returns: Tensor, aux_phase=False):
        clip_vf = self.ppo_cfg.clip_vf if not aux_phase else None
    ...
    def collect_rollouts(self, vec_env: AutoResetVectorEnv, obs: BatchObs):
        """
        Fills the buffer once (buffer_size // N  steps).
        ...
        for step in range(horizon):
            ...
            dist: Categorical = self.policy_net(obs_tensor, ctx_tensor, mask_tensor)   # returns a Categorical        
            actions = dist.sample()
            logps: Tensor   = dist.log_prob(actions)
            logits: Tensor  = dist.logits                              # (N, action_dim)

            # separate VALUE network
            values: Tensor  = self.value_net(obs_tensor, ctx_tensor) 
        ...
        # -------------- stats ---------------------------------
        reward_sum = float(torch.sum(rewards))
        self.int_stats["reward_sum"] += reward_sum
        ...
        last_vals = self.value_net(obs_tensor, ctx_tensor)
    ...
    def _policy_step(self, batch):
        clip_eps  = self.clip_eps
        ent_coef  = self.entropy_coef
        grad_clip = self.grad_clip
        device    = self.device

        # ── batch → device & early check ─────────────────────────────
        ...
        old_logits          = batch["old_logits"].to(device)      # (B, action_dim)

        assert_finite("batch (adv / old_lp)", advs, old_lp)


        # ── forward pass ────────────────────────────────────────────
        dist: Categorical = self.policy_net(obs, ctx, action_masks) 
        assert_finite("policy logits", dist.logits)

        new_lp: Tensor  = dist.log_prob(acts)
        entropy: Tensor = dist.entropy().mean()

        #ratio = torch.exp(torch.clamp(new_lp - old_lp, max=20))  # avoid Inf
        ratio = torch.exp(torch.clamp(new_lp - old_lp, -10, 10))
        assert_finite("ratio / new_lp", ratio, new_lp)
        ...
        loss_policy: Tensor  = actor_loss + entropy_loss
        ...
        # post-update guard: catch NaN params written by Adam
        for p in self.policy_net.parameters():
            if not torch.isfinite(p.data).all():
                raise ValueError("❌ NaN in policy parameters after step()")
        ...
        with torch.no_grad():
            ...
            # ---------- true KL (π_old ‖ π_new) ------------------
            log_pi_old = torch.log_softmax(old_logits, dim=-1)
            log_pi_new = torch.log_softmax(dist.logits, dim=-1)
            pi_old     = torch.softmax(old_logits, dim=-1)
            true_kl_tensor: Tensor = (pi_old * (log_pi_old - log_pi_new)).sum(dim=1).mean()
            # -----------------------------------------------------
        ...
        assert_finite("policy grad norms", torch.tensor(enc_grad), torch.tensor(actor_grad))


        assert_finite("final policy stats", actor_loss, entropy, approx_kl_tensor, true_kl_tensor, clip_frac)
        return {
        "policy_loss": actor_loss.item(),
        "entropy":     entropy.item(),  
        "approx_kl":   approx_kl_tensor.item(),
        "true_kl":     true_kl_tensor.item(),
    ...
    def _critic_step(self, batch):
        ...
        obs     = batch["obs"].to(device)
        ctx     = batch["ctx"].to(device)
        old_val = batch["old_values"].to(device)
        rets    = batch["returns"].to(device)
        assert_finite("critic batch (old_val / rets)", old_val, rets)
        
        # ── forward & loss ─────────────────────────────────────────

        values = self.value_net(obs, ctx)
        assert_finite("critic values", values)

        loss_critic = self.calculate_value_loss(old_values=old_val, values=values, returns=rets, aux_phase=False)

        # ── optimise ───────────────────────────────────────────────
        self.opt_critic.zero_grad(set_to_none=True)
        loss_critic.backward()
        if grad_clip is not None:
            torch.nn.utils.clip_grad_norm_(self.value_net.parameters(), grad_clip)
        self.opt_critic.step()


        # ── diagnostics ────────────────────────────────────────────
        with torch.no_grad():
            var_y   = rets.var(unbiased=False)
            if var_y < 1e-6:                               # --- FIX
                expl_var = torch.tensor(0.0, device=device)
            else:
                var_e = (rets - values).var(unbiased=False)
                expl_var = 1 - var_e / var_y


        assert_finite("final critic stats", loss_critic, expl_var)


        critic_grad = self._grad_l2(self.value_net.value_head.parameters())
        enc_grad    = self._grad_l2(self.value_net.encoder.parameters())
        assert_finite("critic grad norms", torch.tensor(critic_grad))
    ...
    def ppo_phase_update(self): 
        pol_epochs  = self.ppg_cfg.policy_epochs   # E_pi
        crit_epochs = self.ppg_cfg.critic_epochs   # E_V
        batch_size  = self.buf_cfg.ppo_batch_size
        max_epochs  = max(pol_epochs, crit_epochs)

        for epoch in range(max_epochs):
            early_stop = False
            for batch in self.ppo_buf.get_batches(batch_size):

                # --- policy update -----------------------------------------
                if epoch < pol_epochs:
                    pol_stats = self._policy_step(batch)

                # --- critic update -----------------------------------------
                if epoch < crit_epochs:
                    crit_stats = self._critic_step(batch)
                ...
                if self.logging_cfg and self.logging_cfg.log_interval:
                    if epoch < pol_epochs:
                        ...
                        self.list_stats["true_kl"].append(pol_stats["true_kl"])
                        self.list_stats["clip_frac"].append(pol_stats["clip_frac"])
                        self.list_stats["policy_enc_grad"].append(pol_stats["enc_grad"])
                    ...
                    if epoch < crit_epochs:
                        ...
                        self.list_stats["value_enc_grad"].append(crit_stats["enc_grad"])


                    if pol_epochs == 0:
                        for key in ["policy_loss", "entropy", "approx_kl", "true_kl", "clip_frac", "policy_enc_grad", "actor_grad"]:
                            self.list_stats[key].append(0.0)

                if epoch < pol_epochs and self.target_kl is not None and pol_stats["approx_kl"] > self.target_kl:
                    early_stop = True
                    break
    ...
    def auxiliary_phase_update(self):
        """
        Auxiliary phase (PPG):
        • Update policy encoder + aux-value head with
            L = MSE(aux_value, returns) + β · KL(π_old || π_new)
        • Extra MSE passes on stand-alone value_net (V_φ)
        """
        beta     = self.ppg_cfg.beta_kl
        aux_bs   = self.buf_cfg.aux_batch_size
        aux_ep   = self.ppg_cfg.aux_epochs
        grad_clip = self.grad_clip
        device   = self.device

        for epoch in range(aux_ep):
            for batch in self.aux_buf.get_aux_batches(aux_bs):
                obs             = batch["obs"].to(device)
                ctx             = batch["ctx"].to(device)
                action_masks    = batch["action_masks"].to(device)
                old_logits      = batch["old_logits"].to(device)      # (B, A)
                returns         = batch["returns"].to(device)

                # ---------- policy forward (logits + aux value) ---------------
                logits_new, v_aux = self.policy_net(
                    obs, ctx, action_masks, return_aux_value=True
                )                                             # logits_new: (B, A)
                v_aux = v_aux                                 # (B,)

                assert_finite("aux logits", logits_new, old_logits, v_aux)

                # ---------- KL(old || new) ------------------------------------
                pi_old     = torch.softmax(old_logits,  dim=-1)
                log_pi_old = torch.log_softmax(old_logits,  dim=-1)
                log_pi_new = torch.log_softmax(logits_new, dim=-1)
                kl_elem    = (pi_old * (log_pi_old - log_pi_new))      # (B, A)
                kl_elem[~torch.isfinite(kl_elem)] = 0.0                # guard 0*-inf
                kl = kl_elem.sum(dim=1).mean()

                # ---------- Aux-value head loss (no PPO clip) ------------------
                loss_aux_val = self.calculate_value_loss(
                    old_values = v_aux.detach(),       # dummy, clip disabled
                    values     = v_aux,
                    returns    = returns,
                    aux_phase  = True
                )

                # ---------- Joint loss + back-prop into policy -----------------
                loss_joint = loss_aux_val + beta * kl
                self.opt_aux.zero_grad()
                loss_joint.backward()
                if grad_clip is not None:               # --- FIX
                    torch.nn.utils.clip_grad_norm_(self.policy_net.parameters(), grad_clip)
                self.opt_aux.step()

                # ---------- Update stand-alone critic V_φ ----------------------
                v_main = self.value_net(obs, ctx)
                loss_critic = self.calculate_value_loss(
                    old_values = v_main.detach(),   # clip off
                    values     = v_main,
                    returns    = returns,
                    aux_phase  = True
                )
    ...
    def save(self, path: str, file_name: str) -> None:
        ...
        data = {
            ...
            "aux_optimizer": self.opt_aux.state_dict(),
            "critic_optimizer": self.opt_critic.state_dict(),
    ...
    def train_one_update(self, update=None): # added a mask test
        if self.obs is None:
        ...
        # 2) PPO policy phase        
        self.ppo_phase_update()

        # 3) Move that slice to aux_buf, reset ppo_buf
        self.aux_buf.extend(self.ppo_buf)
        self.ppo_buf.clear()
        self._iter_in_phase += 1


        # safety: verify no rows turned illegal during copy
        zeros = (~self.aux_buf.action_masks[:self.aux_buf.pos]).all(dim=1)
        if zeros.any():
            bad = zeros.nonzero(as_tuple=True)[0].tolist()
            raise RuntimeError(f"aux_buf corruption: rows {bad} have all-False action_masks")
        


        # 4) Every N_pi iterations run the auxiliary phase
        if self._iter_in_phase >= self.N_pi:
            if self.ppg_cfg.aux_epochs > 0:
                self.auxiliary_phase_update()   # same method we coded earlier
            self.aux_buf.clear()
            self._iter_in_phase = 0
    ...
    def handle_logs(self, update: int):
        lc = self.logging_cfg
        ec = self.eval_cfg
        if not lc:
            return
    ...
    def evaluate(self, eval_env:AutoResetVectorEnv, n_steps: int = 10000, epsilon: float = 0.3) -> dict:
        ...
        for i in range(steps_needed):
            ...
            # ---------- policy forward -----------------------------------
            dist: Categorical = self.policy_net(obs_tensor, ctx_tensor, mask_tensor)
